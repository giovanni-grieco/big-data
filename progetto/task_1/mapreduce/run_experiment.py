...
def run_and_time(command, file_name):
    ...
    if process.returncode == 0:
        ...
        status = "FALLITO"
        print(f"Errore: {process.stderr.decode('utf-8')}")
...
def main():
    ...
    with open(output_file, "w") as f:
        f.write("dataset,status,execution_time_seconds\n")
        for result in results:
            f.write(f"{result['file_name']},{result['status']},{result['execution_time']:.2f}\n")
